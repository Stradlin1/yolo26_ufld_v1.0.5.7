...
class LaneDetectionModel(DetectionModel):
    """YOLO lane detection model using UFLDHead for coordinate regression.

    This class extends DetectionModel to handle lane detection tasks, using a UFLDHead
    that regresses normalized x-coordinates for fixed row anchors from the P3 feature map.

    Attributes:
        num_rows (int): Number of row anchors (from head).
        row_anchors (list[float]): Fixed y-positions (from head).

    Methods:
        __init__: Initialize the lane detection model.
        init_criterion: Initialize the lane detection loss.

    Examples:
        >>> model = LaneDetectionModel("yolo26-ufld.yaml", ch=3, nc=1)
        >>> results = model.predict(image_tensor)
    """

    def __init__(self, cfg="yolo26-ufld.yaml", ch=3, nc=None, verbose=True):
        """Initialize lane detection model.

        Args:
            cfg (str | dict): Model config path or dict.
    ...
    def init_criterion(self):
        """Initialize the loss criterion for lane detection."""
        return v8LaneDetectionLoss(self)
...
def parse_model(d, ch, verbose=True):
    ...
    num_rows = d.get("num_rows", 4)
    row_anchors = d.get("row_anchors", [1.0, 0.89, 0.78, 0.67])
