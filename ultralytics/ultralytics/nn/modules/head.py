...
class UFLDHead(nn.Module):
    """UFLD lane detection head — spatial softmax over columns per row anchor.

    Instead of pooling width to 1 and regressing via FC (which loses all spatial
    information), this head keeps the full width, applies softmax to get a
    probability distribution over horizontal columns, and returns the expected
    column index as the predicted x-coordinate.

    This forces the model to learn *where* the lane is, rather than guessing
    from a compressed global feature vector.

    Attributes:
        nc (int): Number of classes (always 1 for lane).
        num_rows (int): Number of row anchors (e.g., 56).
        row_anchors (list[float]): Fixed normalized y-positions.
        stride (torch.Tensor): Stride tensor, set during build.

    Methods:
        forward: Returns [B, num_rows] x ∈ [0, 1] via spatial softmax.
        bias_init: No-op.

    Examples:
        >>> ufld = UFLDHead(nc=1, num_rows=56, ch=(256,))
        >>> x = [torch.randn(1, 256, 160, 160)]
        >>> out = ufld(x)  # shape: [1, 56]
    """

    def __init__(self, nc: int = 1, num_rows: int = 4, row_anchors: list[float] | None = None, ch: tuple = ()):
        """Initialize UFLDHead.

        Args:
            nc (int): Number of classes (always 1 for single lane).
            num_rows (int): Number of row anchors.
            row_anchors (list[float]): Fixed y-positions (top-to-bottom, normalized 0~1).
            ch (tuple): Input channel tuple, e.g., (128,) from P2.
        """
        super().__init__()
        self.nc = nc
        self.num_rows = num_rows
        self.row_anchors = row_anchors or [1.0, 0.89, 0.78, 0.67]
        self.stride = torch.zeros(1)

        c_in = ch[0]
        c_mid = max(c_in, 64)

        # Stage 1: 3×3 conv — preserve spatial
        ...
        # Stage 2: 1×1 conv — expand channels
        self.conv2 = nn.Sequential(
            nn.Conv2d(c_mid, 256, 1, bias=False),
            nn.BatchNorm2d(256),
            nn.SiLU(),
        )

        # Stage 3: 3×3 conv — aggregate local context per row (keeps W)
        ...
        # Stage 4: 1×1 conv → num_bins channels → softmax over bins
        self.num_bins = 200
        self.bin_conv = nn.Conv2d(128, self.num_bins, 1)
        self.register_buffer("bin_centers", torch.linspace(0, 1, self.num_bins))

    def forward(self, x: list[torch.Tensor] | torch.Tensor) -> torch.Tensor:
        """Forward: feature map → [B, num_rows] expected x via spatial softmax.

        Args:
            x: Feature map, Tensor [B,C,H,W] or list of one.

        Returns:
            torch.Tensor: [B, num_rows] expected x ∈ [0, 1].
        """
        if isinstance(x, list):
            x = x[0]

        x = self.conv1(x)  # [B, C_mid, H, W]

        # Pool height → num_rows (keep full width for spatial softmax)
        if torch.onnx.is_in_onnx_export():
            x = F.interpolate(x, size=(self.num_rows, x.shape[-1]),
                              mode="bilinear", align_corners=False)
        else:
            x = F.adaptive_avg_pool2d(x, (self.num_rows, x.shape[-1]))
            # → [B, C_mid, num_rows, W]

        x = self.conv2(x)   # [B, 256, num_rows, W]
        x = self.conv3(x)   # [B, 128, num_rows, W] — 3×3 aggregates local context

        # Spatial softmax over fixed bins (ONNX-safe, no dynamic W)
        x = self.bin_conv(x)            # [B, num_bins, num_rows, W]
        x = x.mean(dim=-1)              # [B, num_bins, num_rows] — pool over spatial W
        x = x.permute(0, 2, 1)          # [B, num_rows, num_bins]
        x = F.softmax(x, dim=-1)        # probability distribution over bins

        # Expected x ∈ [0, 1]
        x = (x * self.bin_centers).sum(dim=-1)  # [B, num_rows]
        return x

    def bias_init(self):
        """No-op: classification head has no bias to init."""
        pass
