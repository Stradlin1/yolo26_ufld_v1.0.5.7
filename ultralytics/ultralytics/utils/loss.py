...
class v8LaneDetectionLoss:
    """Criterion class for computing UFLD lane detection training losses.

    Computes two losses:
        L_reg:  Masked L1 loss on valid rows (x != -1).
        L_smooth: Adjacent-row smoothness loss (only between consecutive valid rows).

    Attributes:
        device (torch.device): Device for loss computation.
        hyp (SimpleNamespace): Hyperparameters namespace with keys `lane_reg` and `lane_smooth`.
        stride (torch.Tensor): Model stride.
        num_rows (int): Number of row anchors.
        reg_loss (nn.L1Loss): L1 loss for coordinate regression.
    """

    def __init__(self, model):
        """Initialize lane detection loss with model attributes.

        Args:
            model: LaneDetectionModel instance (de-paralleled).
        """
        ...
        self.num_rows = getattr(model.model[-1], "num_rows", 4)
    ...
    def __call__(self, preds: torch.Tensor, batch: dict[str, torch.Tensor]) -> tuple[torch.Tensor, torch.Tensor]:
        ...
        Args:
            preds (torch.Tensor): [B, num_rows] predicted x-coordinates.
            batch (dict): Batch dict with key 'lane_labels' → [B, num_rows] ground-truth x.

        Returns:
            (torch.Tensor): Total loss (scalar).
            (torch.Tensor): Detached loss tensor [reg, smooth] for logging.
        """
        device = preds.device
        targets = batch["lane_labels"].to(device)  # [B, num_rows], -1 = no lane

        # --- mask: valid rows ---
        mask = targets != -1  # [B, num_rows]

        # --- row weights: emphasize bottom rows (y closer to 1.0) ---
        # Last 15 rows get 2x weight, last 5 rows get 3x weight
        row_weights = torch.ones(self.num_rows, device=device)
        if self.num_rows > 5:
            row_weights[-5:] = 3.0
        if self.num_rows > 15:
            row_weights[-15:-5] = 2.0

        # --- L_reg: masked L1 with row weights ---
        if mask.sum() > 0:
            elem_loss = self.reg_loss(preds[mask], targets[mask])  # [N_valid]
            # Get row indices for masked elements
            row_indices = torch.arange(self.num_rows, device=device).unsqueeze(0).expand_as(mask)[mask]
            weights = row_weights[row_indices]
            loss_reg = (elem_loss * weights).mean()
        else:
            loss_reg = (preds * 0).sum()  # zero grad

        # --- L_smooth: adjacent valid rows (first-order) ---
        adj_mask = mask[:, :-1] & mask[:, 1:]  # [B, num_rows-1]
        if adj_mask.sum() > 0:
            diff = torch.abs(preds[:, :-1] - preds[:, 1:])
            loss_smooth = (diff * adj_mask).sum() / adj_mask.sum().clamp(min=1)
        else:
            loss_smooth = (preds * 0).sum()

        # --- L_smooth2: second-order smoothness (curvature) ---
        # Penalize large changes in slope to reduce jagged/curved predictions
        if self.num_rows >= 3:
            tri_mask = mask[:, :-2] & mask[:, 1:-1] & mask[:, 2:]  # [B, num_rows-2]
            if tri_mask.sum() > 0:
                # second difference: f(i) - 2f(i+1) + f(i+2)
                second_diff = preds[:, :-2] - 2.0 * preds[:, 1:-1] + preds[:, 2:]
                loss_smooth2 = (second_diff.abs() * tri_mask).sum() / tri_mask.sum().clamp(min=1)
            else:
                loss_smooth2 = (preds * 0).sum()
        else:
            loss_smooth2 = (preds * 0).sum()

        # --- combine ---
        w_reg = getattr(self.hyp, "lane_reg", 1.0)
        w_smooth = getattr(self.hyp, "lane_smooth", 0.5)
        w_smooth2 = getattr(self.hyp, "lane_smooth2", 0.1)
        total = w_reg * loss_reg + w_smooth * loss_smooth + w_smooth2 * loss_smooth2

        loss_items = torch.tensor([loss_reg.detach(), loss_smooth.detach(), loss_smooth2.detach()], device=device)
        return total * preds.shape[0], loss_items  # scale by batch size
